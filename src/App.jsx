...
const cities=[
 ...
 {name:'Aurora',number:'03',icon:Palette,role:'Culture',desc:'Art, music, games, design and digital expression.',fields:'Media · music · design',project:'Synthetic Aurora',potential:'A living archive for new cultural forms.',color:'violet'},
 {name:'Genesis',number:'04',icon:Atom,role:'Life Sciences',desc:'Biotechnology, medicine, human development and life research.',fields:'Biotech · medicine · wellbeing',project:'Community BioLab',potential:'Responsible research for longer, healthier lives.',color:'green'},
 {name:'Horizon',number:'05',icon:Rocket,role:'Aerospace',desc:'Spaceflight, satellites, orbital habitats and exploration.',fields:'Satellites · aerospace · habitats',project:'Orbital Garden',potential:'A civilian network for the next space age.',color:'orange'},
 {name:'Quantum',number:'06',icon:Zap,role:'Frontier Science',desc:'Physics, mathematics, computing and advanced technology.',fields:'AI · computing · mathematics',project:'Open Research Mesh',potential:'Shared tools for frontier discovery.',color:'pink'},
...
function Home(){return <Shell><section className="hero"><div className="hero-grid"></div><div className="hero-orbit orbit-one"></div><div className="hero-orbit orbit-two"></div><div className="hero-content"><div className="eyebrow hero-eyebrow"><span className="pulse-dot"></span>A CIVILIZATION FOR THE FUTURE</div><h1>Imagine It.<br/><span>Build It.</span><br/>Live It.</h1><p>NOVA is a digital civilization for people who imagine technologies that do not exist yet—and want a place to turn those ideas into reality.</p><div className="hero-actions"><Link className="primary-btn" to="/citizenship">Become a founding citizen <ArrowUpRight size={17}/></Link><Link className="ghost-btn" to="/innovation">Explore the Dream Factory <ChevronRight size={16}/></Link></div></div><div className="hero-status"><span><CircleDot size={12}/>NOVA NETWORK</span><b>ONLINE</b><span>08 CITIES</span></div></section><section className="ticker"><div>INNOVATION</div><div>SCIENCE</div><div>CREATION</div><div>COLLABORATION</div><div>EXPLORATION</div><div>HUMANITY</div></section><section className="section"><div className="section-wrap"><div className="manifesto"><div className="manifesto-mark">◈</div><div><div className="eyebrow">THE IDEA</div><h2>A home for the future-minded.</h2><p>NOVA connects inventors, scientists, engineers, artists, creators, gamers and entrepreneurs inside one evolving digital nation. It is a place to meet, learn, build, experiment and imagine together.</p></div></div><div className="metrics"><div><strong>08</strong><span>Digital cities</span></div><div><strong>∞</strong><span>Ideas to build</span></div><div><strong>01</strong><span>Shared civilization</span></div><div><strong>◈</strong><span>Nexa economy</span></div></div></div></section><section className="section dark-section"><div className="section-wrap"><SectionHead eyebrow="THE CITIES" title="Eight worlds. One NOVA." link="Explore all cities" to="/cities"/><div className="city-grid">{cities.map(c=><Link className="city-card" to="/cities" key={c.name}><div className="card-top"><span className="city-number">{c.number}</span><c.icon size={21}/></div><span className="city-role">{c.role}</span><h3>{c.name}</h3><p>{c.desc}</p><span className="card-arrow"><ArrowUpRight size={17}/></span></Link>)}</div></div></section><section className="section"><div className="section-wrap"><div className="factory-banner"><div className="factory-copy"><div className="eyebrow">DREAM FACTORY</div><h2>Ideas are citizens too.</h2><p>Every meaningful invention starts somewhere. NOVA gives it a path from imagination to reality.</p><Link className="primary-btn" to="/innovation">Enter Dream Factory <ArrowUpRight size={16}/></Link></div><div className="pipeline-mini">{stages.map((s,i)=><div className="pipeline-node" key={s}><span>0{i+1}</span><b>{s}</b>{i<stages.length-1&&<i/>}</div>)}</div></div></div></section><section className="section dark-section"><div className="section-wrap"><SectionHead eyebrow="FOUNDING PHILOSOPHY" title="The future is a collective project."/><div className="principle-grid">{principles.map((p,i)=><div key={p}><span>0{i+1}</span><h3>{p}</h3></div>)}</div></div></section><section className="section"><div className="section-wrap"><SectionHead eyebrow="CIVILIZATION SYSTEMS" title="A common future, with room to make your own mark."/><div className="systems-grid"><Link to="/economy"><WalletCards/><span>NEXA ECONOMY</span><h3>Creation has a place to flow.</h3><p>Explore a transparent, conceptual economy designed around contribution—not speculation.</p></Link><Link to="/constitution"><Landmark/><span>NOVA COMMONWEALTH</span><h3>Power remains accountable.</h3><p>Read the founding charter for distributed governance, rights and civic participation.</p></Link><Link to="/citizenship"><Users/><span>FOUNDING CITIZENSHIP</span><h3>Enter the founding generation.</h3><p>Begin a prototype identity and help shape the culture of the civilization.</p></Link></div><div className="closing-cta"><div><div className="eyebrow">THE INVITATION</div><h2>Ideas deserve a place to become real.</h2></div><Link className="primary-btn" to="/citizenship">Enter NOVA <ArrowUpRight size={17}/></Link></div></div></section></Shell>}
function Cities(){return <Page eyebrow="NOVA territories" title="Eight cities for eight dimensions of the future." intro="Each city is a digital district with its own communities, projects, knowledge and culture. Together they form the living map of NOVA."><div className="city-grid full-city-grid">{cities.map(c=><article className="city-card" key={c.name}><div className="card-top"><span className="city-number">{c.number}</span><c.icon size={22}/></div><span className="city-role">{c.role}</span><h3>{c.name}</h3><p>{c.desc}</p><div className="city-detail"><span>{c.fields}</span><b>Signal / {c.project}</b><small>{c.potential}</small></div></article>)}</div></Page>}
function Innovation(){return <Page eyebrow="Dream Factory" title="Turn impossible ideas into buildable projects." intro="NOVA's innovation system connects imagination, knowledge, people and resources into a path toward reality."><div className="stage-track">{stages.map((s,i)=><div className={`stage ${i===0?'current':''}`} key={s}><span>0{i+1}</span><b>{s}</b></div>)}</div><div className="innovation-toolbar"><div><div className="eyebrow">DISCOVER</div><h2>Projects being built now</h2></div><button className="outline-btn"><Search size={16}/>Search projects</button></div><div className="project-grid">{projects.map(p=><article className="project-card" key={p.name}><div className="project-meta"><span>{p.city}</span><span>{p.stage}</span></div><h3>{p.name}</h3><p>{p.desc}</p><div className="project-bottom"><span><Users size={15}/>{p.people} builders</span><ArrowUpRight size={17}/></div></article>)}</div><div className="create-panel"><div className="create-icon"><Lightbulb/></div><div><div className="eyebrow">YOUR TURN</div><h2>Have an idea?</h2><p>Create a project with a problem, solution, requirements, team, budget and roadmap.</p></div><button className="primary-btn"><Plus size={17}/>Create project</button></div></Page>}
function Economy(){const funds=[['Treasury','30%'],['Citizen Reserve','20%'],['Innovation Fund','20%'],['Infrastructure','10%'],['Education & Research','10%'],['Founding Institution','5%'],['Emergency Reserve','5%']];return <Page eyebrow="Nexa economy" title="An economy designed to reward creation." intro="Nexa is NOVA's conceptual economic system: value flows toward people who create, solve, teach, research and build."><div className="economy-hero"><div><span className="nexa-symbol">◈</span><div className="eyebrow">CONCEPTUAL CURRENCY</div><h2>Nexa</h2><p>NXA · Maximum authorized supply</p><strong>100,000,000,000</strong></div><div className="economy-orbit"><WalletCards size={36}/><span>◈</span></div></div><div className="economy-grid"><div className="info-panel"><div className="eyebrow">ECONOMIC PRINCIPLE</div><h2>Currency flows toward creation.</h2><p>In the long-term vision, citizens can earn through software, hardware, research, education, art, services, entrepreneurship and other meaningful contributions.</p><div className="earning-grid"><span><Cpu/>Build</span><span><FlaskConical/>Research</span><span><GraduationCap/>Teach</span><span><Palette/>Create</span></div></div><div className="info-panel"><div className="eyebrow">PROPOSED ALLOCATION</div>{funds.map(([name,pct])=><div className="fund-row" key={name}><span>{name}</span><b>{pct}</b><i><em style={{width:pct}}/></i></div>)}</div></div><div className="notice"><Shield size={18}/><span>Nexa is conceptual demonstration data. NOVA does not currently issue cryptocurrency or provide banking, custody, payment or investment services.</span></div></Page>}
function Constitution(){return <Page eyebrow="Founding charter" title="A civilization where people remain sovereign over the future." intro="The NOVA Commonwealth is governed by a founding charter built around creation, accountability, participation and human dignity."><div className="constitution-grid"><div className="charter-panel"><div className="eyebrow">FOUNDING PRINCIPLES</div>{principles.map((p,i)=><div className="charter-row" key={p}><span>0{i+1}</span><b>{p}</b><ChevronRight size={15}/></div>)}</div><div className="charter-panel rights"><div className="eyebrow">CITIZEN RIGHTS</div><h2>Freedom to create.</h2><p>Freedom of creation and association. Access to knowledge. Privacy. Economic participation. Due process. Transparent governance. The right to leave NOVA.</p><div className="charter-note"><Bot size={19}/><span>AI may assist NOVA, but significant civic decisions remain accountable to humans.</span></div></div></div><div className="government"><SectionHead eyebrow="DISTRIBUTED GOVERNANCE" title="Power is designed to be accountable."/><div className="government-grid"><div><Landmark/><b>Citizen Assembly</b><span>House of Citizens + Council of Builders</span></div><div><Shield/><b>Supreme Civic Court</b><span>Rights, disputes and constitutional review</span></div><div><Zap/><b>Nexa Monetary Authority</b><span>Economic system oversight</span></div><div><Bot/><b>AI Authority</b><span>Auditable AI governance and standards</span></div></div></div></Page>}
function Citizenship(){return <Page eyebrow="Citizenship" title="Claim your place in the founding generation." intro="Create a simple founding identity today. Deeper citizenship services will grow with NOVA."><div className="citizenship-layout"><div className="passport-preview"><div className="passport-head"><span>NOVA</span><span>FOUNDING CITIZEN</span></div><div className="passport-mark">N</div><div className="passport-info"><span>DIGITAL PASSPORT</span><strong>FOUNDING CITIZEN</strong><small>Residence · Nexus</small></div><div className="passport-foot"><span>◈ NXA</span><span>NOVA COMMONWEALTH</span></div></div><div className="signup-panel"><div className="eyebrow">FOUNDING REGISTRATION</div><h2>Enter the future.</h2><label>Display name<input placeholder="How should NOVA know you?"/></label><label>Email<input type="email" placeholder="you@example.com"/></label><label>What do you want to build?<input placeholder="Robotics, music, climate tech..."/></label><button className="primary-btn">Create NOVA identity <ArrowUpRight size={16}/></button><p className="form-note">MVP registration only. Authentication and persistent identity services will connect as the platform evolves.</p></div></div></Page>}
function Dashboard(){return <Page eyebrow="Citizen OS" title="Welcome to NOVA." intro="Your starting point inside the digital civilization."><div className="dashboard-top"><div className="passport-small"><div className="passport-small-mark">N</div><div><span>FOUNDING CITIZEN</span><h2>Pioneer</h2><p>Residence · Nexus</p></div><span className="online">ONLINE</span></div><div className="dashboard-action"><div className="eyebrow">NEXT MOVE</div><h3>Build something.</h3><p>Your next invention starts as an idea.</p><Link className="primary-btn" to="/innovation">Create project <ArrowUpRight size={16}/></Link></div></div><div className="os-grid"><div className="os-card"><WalletCards/><span>◈ NEXA WALLET</span><strong>1,000 ◈</strong><small>Demo balance</small></div><div className="os-card"><Lightbulb/><span>PROJECTS</span><strong>03</strong><small>Active builds</small></div><div className="os-card"><Users/><span>COMMUNITIES</span><strong>07</strong><small>Joined</small></div><div className="os-card"><Landmark/><span>CIVIC</span><strong>00</strong><small>Proposals</small></div></div><div className="dashboard-lower"><div className="info-panel"><div className="eyebrow">YOUR NOVA</div><h2>Digital life, one place.</h2><div className="os-links"><Link to="/cities">Cities <ChevronRight/></Link><Link to="/innovation">Projects <ChevronRight/></Link><Link to="/economy">Economy <ChevronRight/></Link><Link to="/constitution">Civic <ChevronRight/></Link></div></div><div className="info-panel dashboard-quote"><span>“</span><p>The future belongs not only to those who predict it, but to those willing to build it.</p></div></div></Page>}
function App(){return <Routes><Route path="/" element={<Home/>}/><Route path="/cities" element={<Cities/>}/><Route path="/innovation" element={<Innovation/>}/><Route path="/economy" element={<Economy/>}/><Route path="/constitution" element={<Constitution/>}/><Route path="/citizenship" element={<Citizenship/>}/><Route path="/dashboard" element={<Dashboard/>}/><Route path="*" element={<Home/>}/></Routes>}
export default App;
